...
const CHAPTER_26_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Trasformare ogni concorso in capitale di studio" })
      .first()
      .click()
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter26Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-26"))
      const visibleChapter26Images = chapter26Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter26Images.map((image, index) => {
    ...
    if (diag.chapter26Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-26, trovate ${diag.chapter26Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-26 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_26_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-26, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_26_ASSETS, file), "utf8")
