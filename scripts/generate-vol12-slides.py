"""Genera i deck HTML VOL-12 dai capitoli wiki (M-SP01–04 Carriere speciali)."""
...
SLIDES = ROOT / "slides" / "VOL-12"
VOLUME = "VOL-12"

MODULES = [
    {
        "code": "M-SP01",
        "book_id": "m-sp01-forze-ordine",
        "dir": WIKI / "moduli" / "m-sp01-forze-ordine" / "chapters",
        "files": [
            "01-mappa-famiglia-struttura-selezione.md",
            "02-posizione-prima-domanda.md",
            "03-formati-prova-scritta.md",
            "04-accertamenti-preparazione.md",
            "05-prova-orale-titoli-lingua.md",
            "06-materie-riuso-specialistiche.md",
            "07-bando-decoder.md",
            "08-piano-30-60-90-doppio-binario.md",
            "09-errori-casi-guidati.md",
            "10-checklist-finale.md",
        ],
    },
    {
        "code": "M-SP02",
        "book_id": "m-sp02-vigili-fuoco",
        "dir": WIKI / "moduli" / "m-sp02-vigili-fuoco" / "chapters",
        "files": [
            "01-mappa-della-famiglia.md",
            "02-la-tua-posizione-prima-della-domanda.md",
            "03-la-preselezione.md",
            "04-tre-prove-motorio-attitudinali.md",
            "05-prove-direttive-e-specializzazione.md",
            "06-riuso-e-bando-decoder.md",
            "07-piano-30-60-90.md",
            "08-errori-casi-checklist.md",
        ],
    },
    {
        "code": "M-SP03",
        "book_id": "m-sp03-magistratura-avvocatura-notariato",
        "dir": WIKI / "moduli" / "m-sp03-magistratura-avvocatura-notariato" / "chapters",
        "files": [
            "01-mappa-scelta-binario.md",
            "02-magistratura-prove-ordinamento.md",
            "03-avvocatura-stato-prove-ordinamento.md",
            "04-notariato-prove-ordinamento.md",
            "05-metodo-prove-scritte.md",
            "06-piano-pluriennale.md",
            "07-errori-casi-checklist.md",
        ],
    },
    {
        "code": "M-SP04",
        "book_id": "m-sp04-prefettizia-diplomatica",
        "dir": WIKI / "moduli" / "m-sp04-prefettizia-diplomatica" / "chapters",
        "files": [
            "01-mappa-scelta-bando-decoder.md",
            "02-carriera-prefettizia-prove-materie-ordinamento.md",
            "03-carriera-diplomatica-prove-materie-ordinamento.md",
            "04-le-lingue-straniere.md",
            "05-prova-orale-postura-professionale.md",
            "06-piano-preparazione-tentativi.md",
            "07-errori-casi-checklist.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    title = re.sub(r"^Capitolo\s+\d+\s+[—–-]\s+", "", title)
    ...
    obiettivo = find_section(sections, "obiettivo del capitolo", "obiettivo operativo", "obiettivo")
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-12 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-12", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-12", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-12", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-12", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-12", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-12", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-12 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-12 · Fine</h2>
...
<head>
    ...
    <title>VOL-12 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol12-{n}" data-volume="VOL-12" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-12 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-12 · Indice volume compilato</p>
            <h1>Carriere speciali premium</h1>
        </div>
    </header>
    <main>
        <p class="meta"><a href="../index.html" style="color:#D4AF37;text-decoration:none;">← Catalogo</a> · Cartella = numero volume + slug file wiki · 01–32 pronti</p>
        <ol class="chapters">
{chr(10).join(items)}
        </ol>
        <footer>01–10 M-SP01 forze dell’ordine · 11–18 M-SP02 vigili del fuoco · 19–25 M-SP03 magistratura/avvocatura/notariato · 26–32 M-SP04 prefettizia/diplomatica</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "spec": "wiki/books/volumi/vol-12-carriere-speciali-premium/planning/00-scheda-pipeline.md",
        "order": "M-SP01 01–10 · M-SP02 11–18 · M-SP03 19–25 · M-SP04 26–32",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-12 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-12")
