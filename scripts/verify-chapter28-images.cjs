...
const CHAPTER_28_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Usare il digitale senza perdere il metodo" })
      .first()
      .click()
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 28")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter28-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter28Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-28"))
      const visibleChapter28Images = chapter28Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter28Images.map((image, index) => {
    ...
    if (diag.chapter28Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-28, trovate ${diag.chapter28Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-28 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_28_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-28, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_28_ASSETS, file), "utf8")
