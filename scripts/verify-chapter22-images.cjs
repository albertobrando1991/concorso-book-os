...
const CHAPTER_22_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Piano 30/60/90 giorni" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 22")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter22-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter22Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-22"))
      const visibleChapter22Images = chapter22Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter22Images.map((image, index) => {
    ...
    if (diag.chapter22Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-22, trovate ${diag.chapter22Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-22 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_22_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-22, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_22_ASSETS, file), "utf8")
