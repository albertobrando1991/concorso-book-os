"""Genera i deck HTML VOL-10 dai capitoli wiki (M-TR03 Tecnico-ingegneristico)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-tr03-tecnico-ingegneristico" / "chapters",
        "files": [
            "01-concorso-tecnico-pa-profili-prove.md",
            "02-ufficio-tecnico-responsabilita-atti.md",
            "03-scienza-tecnica-costruzioni.md",
            "04-ntc-sismica-geotecnica-sicurezza-strutturale.md",
            "05-urbanistica-governo-territorio.md",
            "06-edilizia-sue-titoli-vigilanza.md",
            "07-progettazione-opere-pubbliche.md",
            "08-direzione-lavori-esecuzione.md",
            "09-collaudo-verifica-manutenzione.md",
            "10-computi-capitolati-contabilita-lavori.md",
            "11-infrastrutture-viabilita-ponti-monitoraggio.md",
            "12-bim-gis-catasto-patrimonio.md",
            "13-laboratorio-prove-tecniche.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-10 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-10", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-10", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-10", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-10", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-10", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-10", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-10 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-10 · Fine</h2>
...
<head>
    ...
    <title>VOL-10 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol10-{n}" data-volume="VOL-10" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-10 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-10 · Indice volume compilato</p>
            <h1>Tecnico-ingegneristico, territorio, lavori pubblici</h1>
        </div>
    </header>
    <main>
        <p class="meta"><a href="../index.html" style="color:#D4AF37;text-decoration:none;">← Catalogo</a> · Cartella = numero volume + slug file wiki · 01–13 pronti</p>
        <ol class="chapters">
{chr(10).join(items)}
        </ol>
        <footer>01–02 profilo/ufficio · 03–06 costruzioni/territorio · 07–10 opere/DL/contabilità · 11–12 infrastrutture/BIM · 13 laboratorio</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        "volume": "VOL-10",
        "volume_title": "Tecnico-ingegneristico, territorio, lavori pubblici",
        "spec": "wiki/books/volumi/vol-10-tecnico-ingegneristico-territorio-lavori-pubblici/planning/00-scheda-pipeline.md",
        "order": "modulo unico M-TR03, capitoli 01–13",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-10 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-10")
