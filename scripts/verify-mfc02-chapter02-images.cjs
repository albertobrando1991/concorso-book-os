...
const CHAPTER_PATH = "books/moduli/m-fc02-agenzie-fiscali/chapters/02-bando-decoder-fiscale.md"
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      const chapterImages = images.filter((image) => {
        ...
        return decoded.includes("m-fc02-agenzie-fiscali/assets/chapter-02")
      })
    ...
    if (pageCount < 6) {
      throw new Error("Meno di 6 pagine nella preview del capitolo 2 M-FC02")
    }

    await pages.nth(0).screenshot({ path: "artifacts/mfc02-chapter02-page1-preview.png" })
    ...
    if (diag.chapterImages !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini M-FC02 chapter-02, trovate ${diag.chapterImages}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini M-FC02 chapter-02 non conformi nella pagina: ${JSON.stringify({
        pages: diag.pages,
...
async function auditSvgTextGeometry(browser) {
  ...
  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG M-FC02 chapter-02, trovati ${files.length}`)
  }
  ...
  if (failures.length > 0) {
    throw new Error(`Testi fuori margine negli SVG M-FC02 chapter-02: ${JSON.stringify(failures, null, 2)}`)
  }
