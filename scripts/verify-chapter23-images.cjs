...
const CHAPTER_23_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Il diario degli errori" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 23")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter23-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter23Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-23"))
      const visibleChapter23Images = chapter23Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter23Images.map((image, index) => {
    ...
    if (diag.chapter23Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-23, trovate ${diag.chapter23Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-23 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_23_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-23, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_23_ASSETS, file), "utf8")
