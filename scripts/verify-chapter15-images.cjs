...
const CHAPTER_15_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /15\.\s*La prova scritta/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 15")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter15-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter15Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-15"))
      const visibleChapter15Images = chapter15Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter15Images.map((image, index) => {
    ...
    if (diag.chapter15Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-15, trovate ${diag.chapter15Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-15 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_15_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-15, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_15_ASSETS, file), "utf8")
