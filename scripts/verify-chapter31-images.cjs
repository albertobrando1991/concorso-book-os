...
const CHAPTER_31_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Prendere servizio" })
      .first()
      .click()
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 31")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter31-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter31Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-31"))
      const visibleChapter31Images = chapter31Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter31Images.map((image, index) => {
    ...
    if (diag.chapter31Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-31, trovate ${diag.chapter31Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-31 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_31_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-31, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_31_ASSETS, file), "utf8")
