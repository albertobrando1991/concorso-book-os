...
const CHAPTER_16_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /16\.\s*La prova orale/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 16")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter16-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter16Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-16"))
      const visibleChapter16Images = chapter16Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter16Images.map((image, index) => {
    ...
    if (diag.chapter16Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-16, trovate ${diag.chapter16Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-16 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_16_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-16, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_16_ASSETS, file), "utf8")
