...
const CHAPTER_24_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Checklist operative" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 24")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter24-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter24Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-24"))
      const visibleChapter24Images = chapter24Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter24Images.map((image, index) => {
    ...
    if (diag.chapter24Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-24, trovate ${diag.chapter24Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-24 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_24_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-24, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_24_ASSETS, file), "utf8")
