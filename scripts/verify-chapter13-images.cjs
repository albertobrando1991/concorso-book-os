...
const CHAPTER_13_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /13\.\s*Metodo di studio/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 13")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter13-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter13Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-13"))
      const visibleChapter13Images = chapter13Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter13Images.map((image, index) => {
    ...
    if (diag.chapter13Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-13, trovate ${diag.chapter13Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-13 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_13_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-13, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_13_ASSETS, file), "utf8")
