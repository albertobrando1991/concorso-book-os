"""Genera i deck HTML VOL-06 dai capitoli wiki (M-IR01–04 Scuola, Università, Ricerca, Cultura)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-ir01-scuola" / "chapters",
        "files": [
            "01-scuola-quattro-profili.md",
            "02-sistema-nazionale-autonomia-organizzazione.md",
            "03-organi-collegiali-governance.md",
            "04-ptof-rav-pdm-valutazione-inclusione.md",
            "05-segreteria-ata-alunni-protocollo-carriere.md",
            "06-dsga-eq-ruolo-uffici-personale.md",
            "07-contabilita-scolastica.md",
            "08-acquisti-inventario-pnrr.md",
            "09-dirigente-scolastico-leadership-management.md",
            "10-relazioni-sindacali-sicurezza-responsabilita.md",
            "11-pedagogia-psicologia-inclusione.md",
            "12-metodologie-valutazione-digitale-didattico.md",
            "13-progettazione-lezione-simulata.md",
        ],
    },
    {
        "code": "M-IR02",
        "book_id": "m-ir02-universita-afam",
        "dir": WIKI / "moduli" / "m-ir02-universita-afam" / "chapters",
        "files": [
            "01-sistema-universitario-afam.md",
            "02-autonomia-statuti-organi.md",
            "03-ordinamenti-didattici-qualita.md",
            "04-carriere-studenti-diritto-studio.md",
            "05-atti-protocollo-accesso-digitale.md",
            "06-bilancio-ateneo.md",
            "07-ricerca-grant-management.md",
            "08-prin-horizon-pnrr-audit.md",
            "09-biblioteche-cataloghi-open-access.md",
            "10-orientamento-placement-terza-missione.md",
            "11-afam-ordinamento-amministrazione.md",
            "12-laboratorio-quattro-profili.md",
        ],
    },
    {
        "code": "M-IR03",
        "book_id": "m-ir03-enti-ricerca",
        "dir": WIKI / "moduli" / "m-ir03-enti-ricerca" / "chapters",
        "files": [
            "01-epr-quattro-profili.md",
            "02-autonomia-statuti-governance.md",
            "03-profili-organizzazione.md",
            "04-amministrazione-contabilita-controlli.md",
            "05-compliance-audit-acquisti-missioni.md",
            "06-metodologia-progettazione.md",
            "07-pubblicazioni-valutazione-integrita.md",
            "08-tecnologo-laboratori-infrastrutture.md",
            "09-proprieta-intellettuale-trasferimento.md",
            "10-open-science-fair-dati.md",
            "11-horizon-pnrr-grant-management.md",
            "12-laboratorio-quattro-profili.md",
        ],
    },
    {
        ...
        "dir": WIKI / "moduli" / "m-ir04-cultura-beni-culturali" / "chapters",
        "files": [
            "01-mic-quattro-profili.md",
            "02-organizzazione-centrale-periferica.md",
            "03-codice-beni-culturali-sistema-categorie.md",
            "04-tutela-valorizzazione-fruizione-paesaggio.md",
            "05-procedimenti-vincoli-circolazione.md",
            "06-catalogazione-digitalizzazione.md",
            "07-archivistica-archivi.md",
            "08-biblioteconomia-servizi.md",
            "09-archeologia-tutela-territoriale.md",
            "10-storia-arte-museologia.md",
            "11-restauro-conservazione.md",
            "12-architettura-paesaggio-cantieri.md",
            "13-fruizione-vigilanza-sicurezza.md",
        ],
    },
]
...
def content_sections(sections: list[tuple[str, str]]) -> list[tuple[str, str]]:
    ...
    for title, body in sections:
        ...
        if (nucleo or (h3s and len(h3s) >= 4)) and h3s:
            for h3t, h3b in h3s:
            ...
            continue
        if nucleo:
            out.append((re.sub(r"^N-[A-Z0-9]+-\d+-\d+\s*·\s*", "", title, flags=re.I), body))
            continue
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    if not sections:
        sections = split_h3(body)
    else:
        first = re.search(r"(?m)^## ", body)
        if first:
            pre = split_h3(body[: first.start()])
            if pre:
                sections = pre + sections
    apertura = find_section(sections, "apertura editoriale")
    obiettivo = find_section(sections, "obiettivo del capitolo", "obiettivo operativo")
    bando = find_section(sections, "mappa bando")
    caso = find_section(sections, "caso guidato", "caso completo")
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-06 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-06", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-06", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-06", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-06", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-06", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-06", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-06 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-06 · Fine</h2>
...
<head>
    ...
    <title>VOL-06 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol06-{n}" data-volume="VOL-06" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-06 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-06 · Indice volume compilato</p>
            <h1>Scuola, Università, Ricerca e Cultura</h1>
        </div>
    </header>
    <main>
...
{chr(10).join(items)}
        ...
        <footer>01–13 M-IR01 · 14–25 M-IR02 · 26–37 M-IR03 · 38–50 M-IR04</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "spec": "wiki/books/volumi/vol-06-scuola-universita-ricerca-cultura/planning/00-scheda-pipeline.md",
        "order": "M-IR01, M-IR02, M-IR03, M-IR04 — 50 capitoli",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-06 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-06")
