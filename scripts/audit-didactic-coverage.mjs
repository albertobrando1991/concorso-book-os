...
function precise(destination) { const value = destination.trim(); if (!value || /^(?:volume base|altrove|vedi sopra)$/i.test(value)) return false; if (/\[\[[^\]]+#[^\]]+\]\]/.test(value)) return true; const pathTarget = /\[\[[^\]]+\]\]/.test(value) || /(?:^|\s)(?:\.{0,2}\/)?[\w./-]+\.md\b/i.test(value), humanTarget = /(?:\bVOL-\d+\b|\bM-[A-Z]{2}\d+\b|\bvolume\s+\d+\b)/i.test(value), chapter = /\bcap(?:itolo)?\.?\s*[\w-]+/i.test(value), paragraph = /(?:§|\bpar(?:agrafo)?\.?)\s*[\w-]+/i.test(value); return (pathTarget && (chapter || paragraph)) || (humanTarget && chapter && paragraph) }
function line(text) { const value = text.trim(); return value.startsWith("|") && value.endsWith("|") ? tokenize(value.slice(1, -1)) : null }
function tokenize(value) { const cells = []; let cell = "", delimiter = 0, wikilink = false; for (let i = 0; i < value.length; i += 1) { const char = value[i], next = value[i + 1]; if (char === "\\" && next === "|") { cell += "|"; i += 1; continue } if (!delimiter && char === "[" && next === "[") { wikilink = true; cell += "[["; i += 1; continue } if (!delimiter && wikilink && char === "]" && next === "]") { wikilink = false; cell += "]]"; i += 1; continue } if (!wikilink && char === "`") { let run = 1; while (value[i + run] === "`") run += 1; if (!delimiter) delimiter = run; else if (run === delimiter) delimiter = 0; cell += "`".repeat(run); i += run - 1; continue } if (char === "|" && !delimiter && !wikilink) { cells.push(cell.trim()); cell = "" } else cell += char } cells.push(cell.trim()); return cells }
