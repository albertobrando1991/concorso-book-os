...
const CHAPTER_19_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.locator("button.chapterButton").filter({ hasText: "Le famiglie dei concorsi pubblici" }).first().click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 19")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter19-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter19Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-19"))
      const visibleChapter19Images = chapter19Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter19Images.map((image, index) => {
    ...
    if (diag.chapter19Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-19, trovate ${diag.chapter19Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-19 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_19_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-19, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_19_ASSETS, file), "utf8")
