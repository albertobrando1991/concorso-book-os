...
const CHAPTER_PATH = "books/moduli/m-fc02-agenzie-fiscali/chapters/07-riscossione-nazionale-lavoro-ader.md"
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      const chapterImages = images.filter((image) => {
        ...
        return decoded.includes("m-fc02-agenzie-fiscali/assets/chapter-07")
      })
    ...
    if (pageCount < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 7 M-FC02")
    }

    await pages.nth(0).screenshot({ path: "output/playwright/mfc02-chapter07-page1-preview.png" })
    ...
    if (diag.chapterImages !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini M-FC02 chapter-07, trovate ${diag.chapterImages}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini M-FC02 chapter-07 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues, null, 2)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  ...
  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG M-FC02 chapter-07, trovati ${files.length}`)
  }
