...
const CHAPTER_17_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.locator("button.chapterButton").filter({ hasText: /^17\. Casi pratici/ }).first().click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 17")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter17-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter17Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-17"))
      const visibleChapter17Images = chapter17Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter17Images.map((image, index) => {
    ...
    if (diag.chapter17Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-17, trovate ${diag.chapter17Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-17 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_17_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-17, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_17_ASSETS, file), "utf8")
