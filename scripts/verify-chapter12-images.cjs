...
const CHAPTER_12_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /12\.\s*Logica/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 12")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter12-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter12Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-12"))
      const visibleChapter12Images = chapter12Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter12Images.map((image, index) => {
    ...
    if (diag.chapter12Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-12, trovate ${diag.chapter12Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-12 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_12_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-12, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_12_ASSETS, file), "utf8")
