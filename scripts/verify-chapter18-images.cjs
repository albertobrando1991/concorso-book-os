...
const CHAPTER_18_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.locator("button.chapterButton").filter({ hasText: /^18\. Quesiti situazionali/ }).first().click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 18")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter18-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter18Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-18"))
      const visibleChapter18Images = chapter18Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter18Images.map((image, index) => {
    ...
    if (diag.chapter18Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-18, trovate ${diag.chapter18Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-18 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_18_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-18, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_18_ASSETS, file), "utf8")
