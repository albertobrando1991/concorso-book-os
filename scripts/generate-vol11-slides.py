"""Genera i deck HTML VOL-11 dai capitoli wiki (M-TR04 Ambiente e protezione civile)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-tr04-ambiente-protezione-civile" / "chapters",
        "files": [
            "01-quattro-profili-mappa-sistema.md",
            "02-dlgs-152-mase-ispra-snpa.md",
            "03-via-vas-valutazioni-ambientali.md",
            "04-aia-aua-emissioni-autorizzazioni.md",
            "05-acque-scarichi-servizio-idrico.md",
            "06-rifiuti-economia-circolare-rentri.md",
            "07-bonifiche-siti-contaminati-danno.md",
            "08-aria-rumore-monitoraggio-dati.md",
            "09-controlli-sanzioni-reati-ambientali.md",
            "10-sistema-protezione-civile-pianificazione.md",
            "11-rischi-allertamento-it-alert-emergenze.md",
            "12-clima-energia-rinnovabili-cer.md",
            "13-dnsh-cam-sostenibilita-pa.md",
            "14-laboratorio-casi-quesiti-sintetici.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-11 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-11", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-11", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-11", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-11", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-11", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-11", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-11 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-11 · Fine</h2>
...
<head>
    ...
    <title>VOL-11 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol11-{n}" data-volume="VOL-11" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-11 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-11 · Indice volume compilato</p>
            <h1>Ambiente, protezione civile e sostenibilità</h1>
        </div>
    </header>
    <main>
        <p class="meta"><a href="../index.html" style="color:#D4AF37;text-decoration:none;">← Catalogo</a> · Cartella = numero volume + slug file wiki · 01–14 pronti</p>
        <ol class="chapters">
{chr(10).join(items)}
        </ol>
        <footer>01–02 profili/sistema · 03–09 valutazioni/autorizzazioni/controlli · 10–11 protezione civile · 12–13 clima/DNSH · 14 laboratorio</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        "volume": "VOL-11",
        "volume_title": "Ambiente, protezione civile e sostenibilità",
        "spec": "wiki/books/volumi/vol-11-ambiente-protezione-civile-sostenibilita/planning/00-scheda-pipeline.md",
        "order": "modulo unico M-TR04, capitoli 01–14",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-11 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-11")
