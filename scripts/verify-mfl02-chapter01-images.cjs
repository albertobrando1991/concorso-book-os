...
const BOOK_ID = "moduli/m-fl02-regioni-province-citta-metropolitane"
const CHAPTER_PATH = "books/moduli/m-fl02-regioni-province-citta-metropolitane/chapters/01-il-sistema-territoriale-multilivello.md"
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      const chapterImages = images.filter((image) => {
        ...
        return decoded.includes("m-fl02-regioni-province-citta-metropolitane/assets/chapter-01")
      })
    ...
    if (pageCount < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 1 M-FL02")
    }

    await pages.nth(0).screenshot({ path: "artifacts/mfl02-chapter01-page1-preview.png" })
    ...
    if (diag.chapterImages !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini M-FL02 chapter-01, trovate ${diag.chapterImages}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini M-FL02 chapter-01 non conformi nella pagina: ${JSON.stringify({
        pages: diag.pages,
...
async function auditSvgTextGeometry(browser) {
  ...
  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG M-FL02 chapter-01, trovati ${files.length}`)
  }
  ...
  if (failures.length > 0) {
    throw new Error(`Testi fuori margine negli SVG M-FL02 chapter-01: ${JSON.stringify(failures, null, 2)}`)
  }
