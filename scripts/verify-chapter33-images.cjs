...
const CHAPTER_33_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 33")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter33-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter33Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-33"))
      const visibleChapter33Images = chapter33Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter33Images.map((image, index) => {
    ...
    if (diag.chapter33Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-33, trovate ${diag.chapter33Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-33 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_33_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-33, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_33_ASSETS, file), "utf8")
