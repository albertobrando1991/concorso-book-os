...
const CHAPTER_30_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 30")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter30-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter30Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-30"))
      const visibleChapter30Images = chapter30Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter30Images.map((image, index) => {
    ...
    if (diag.chapter30Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-30, trovate ${diag.chapter30Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-30 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_30_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-30, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_30_ASSETS, file), "utf8")
