...
const CHAPTER_14_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /14\.\s*La prova a quiz/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 14")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter14-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter14Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-14"))
      const visibleChapter14Images = chapter14Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter14Images.map((image, index) => {
    ...
    if (diag.chapter14Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-14, trovate ${diag.chapter14Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-14 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_14_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-14, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_14_ASSETS, file), "utf8")
