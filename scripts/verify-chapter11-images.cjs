...
const CHAPTER_11_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page.getByRole("button", { name: /11\.\s*Inglese/i }).click()
    await page.waitForTimeout(2500)

    const pages = page.locator(".bookPages > .bookPage")
    const count = await pages.count()
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 11")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter11-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter11Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-11"))
      const visibleChapter11Images = chapter11Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter11Images.map((image, index) => {
    ...
    if (diag.chapter11Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-11, trovate ${diag.chapter11Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-11 non conformi nella pagina A4: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_11_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-11, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_11_ASSETS, file), "utf8")
