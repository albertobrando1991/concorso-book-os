...
const BOOK_ID = "moduli/m-fl03-camere-commercio"
const CHAPTER_PATH = "books/moduli/m-fl03-camere-commercio/chapters/01-camere-commercio-sistema-camerale-unioncamere.md"
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      const chapterImages = images.filter((image) => {
        ...
        return decoded.includes("m-fl03-camere-commercio/assets/chapter-01")
      })
    ...
    if (pageCount < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 1 M-FL03")
    }

    await pages.nth(0).screenshot({ path: "artifacts/mfl03-chapter01-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      return {
        ...
        measureState: (() => {
          const visiblePage = document.querySelector(".bookPages > .bookPage")
          const measurePage = document.querySelector(".paginationMeasurePage")
          const visibleRect = visiblePage ? visiblePage.getBoundingClientRect() : null
          const measureRect = measurePage ? measurePage.getBoundingClientRect() : null

          return {
            visibleWidth: visibleRect ? Math.round(visibleRect.width) : null,
            visibleHeight: visibleRect ? Math.round(visibleRect.height) : null,
            measureWidth: measureRect ? Math.round(measureRect.width) : null,
            measureHeight: measureRect ? Math.round(measureRect.height) : null,
            measureStyleWidth: measurePage ? measurePage.style.width : "",
            measureStyleHeight: measurePage ? measurePage.style.height : ""
          }
        })(),
        overlapPages: pageEls
          .map((el, index) => {
            ...
            return {
              ...
              blocks: el.querySelectorAll(".previewBlocks > *").length,
              headings: Array.from(el.querySelectorAll(".previewBlocks h4, .previewBlocks h5"))
    ...
    if (diag.chapterImages !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini M-FL03 chapter-01, trovate ${diag.chapterImages}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini M-FL03 chapter-01 non conformi nella pagina: ${JSON.stringify({
        pages: diag.pages,
        measureState: diag.measureState,
        overlapPages: diag.overlapPages,
        imageLayoutIssues: diag.imageLayoutIssues
      })}`)
    }
    if (diag.overlapPages.length > 0) {
      throw new Error(`Contenuto sovrapposto al footer: ${JSON.stringify({
        pages: diag.pages,
        measureState: diag.measureState,
        overlapPages: diag.overlapPages
      })}`)
    ...
    const firstImagePage = diag.imageLayouts[0] ? diag.imageLayouts[0].page : null
    if (firstImagePage) {
      await pages.nth(firstImagePage - 1).screenshot({ path: "artifacts/mfl03-chapter01-first-image-page-preview.png" })
    }
...
async function auditSvgTextGeometry(browser) {
  ...
  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG M-FL03 chapter-01, trovati ${files.length}`)
  }
  ...
  if (failures.length > 0) {
    throw new Error(`Testi fuori margine negli SVG M-FL03 chapter-01: ${JSON.stringify(failures, null, 2)}`)
  }
