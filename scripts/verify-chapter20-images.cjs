...
const CHAPTER_20_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Mappe profilo: cosa resta comune e cosa cambia" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 20")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter20-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter20Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-20"))
      const visibleChapter20Images = chapter20Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter20Images.map((image, index) => {
    ...
    if (diag.chapter20Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-20, trovate ${diag.chapter20Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-20 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_20_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-20, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_20_ASSETS, file), "utf8")
