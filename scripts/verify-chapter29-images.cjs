...
const CHAPTER_29_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Reggere la preparazione" })
      .first()
      .click()
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 29")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter29-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter29Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-29"))
      const visibleChapter29Images = chapter29Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter29Images.map((image, index) => {
    ...
    if (diag.chapter29Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-29, trovate ${diag.chapter29Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-29 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_29_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-29, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_29_ASSETS, file), "utf8")
