"""Genera i deck HTML VOL-08 dai capitoli wiki (M-TR01 ICT e trasformazione digitale)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-tr01-ict-trasformazione-digitale" / "chapters",
        "files": [
            "01-lavorare-ict-pa-ruoli-enti-prove.md",
            "02-informatica-specialistica-oltre-vol-01.md",
            "03-programmazione-algoritmi-strutture-dati.md",
            "04-basi-dati-sql-nosql-qualita-dato.md",
            "05-reti-sistemi-operativi-infrastrutture.md",
            "06-ingegneria-software-api-interoperabilita-pa.md",
            "07-cloud-pa-virtualizzazione-container-devops.md",
            "08-cybersecurity-rischio-controlli-vulnerabilita.md",
            "09-iam-crittografia-logging-incident-response.md",
            "10-data-governance-open-data-interoperabilita.md",
            "11-ai-ml-pa-rischi-compliance.md",
            "12-procurement-ict-gestione-fornitori.md",
            "13-laboratorio-prove-ict.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-08 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-08", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-08", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-08", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-08", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-08", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-08", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-08 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-08 · Fine</h2>
...
<head>
    ...
    <title>VOL-08 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol08-{n}" data-volume="VOL-08" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-08 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-08 · Indice volume compilato</p>
            <h1>ICT, digitale, cybersecurity e dati</h1>
        </div>
    </header>
    <main>
...
{chr(10).join(items)}
        ...
        <footer>01–02 profilo · 03–07 infrastrutture · 08–09 cyber · 10–12 dati/AI/fornitori · 13 laboratorio</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "spec": "wiki/books/volumi/vol-08-ict-digitale-cybersecurity-dati/planning/00-scheda-pipeline.md",
        "order": "modulo unico M-TR01, capitoli 01–13",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-08 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-08")
