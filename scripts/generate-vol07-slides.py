"""Genera i deck HTML VOL-07 dai capitoli wiki (M-SA01–04 Sanità)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-sa01-sanita-amministrativa" / "chapters",
        "files": [
            "04-atti-procedimenti-flussi-informativi.md",
            "05-documentazione-accesso-conservazione.md",
            "06-front-office-comunicazione-utenza.md",
            "09-contabilita-budget-controllo-gestione.md",
            "10-procurement-farmaci-dispositivi-magazzino.md",
        ],
    },
    {
        "code": "M-SA02",
        "book_id": "m-sa02-professioni-sanitarie",
        "dir": WIKI / "moduli" / "m-sa02-professioni-sanitarie" / "chapters",
        "files": [
            "01-mappa-profili-e-prove.md",
            "03-discipline-professionali-autonomia-responsabilita.md",
            "04-assistenza-infermieristica-tecniche-assistenziali-oss.md",
            "05-valutazione-clinica-triage-urgenza-emergenza.md",
            "06-prevenzione-continuita-presa-in-carico.md",
            "07-evidenze-pico-grade-applicabilita.md",
            "08-igiene-pubblica-epidemiologia-screening.md",
            "09-controlli-tpall-verbalizzazione-campionamento-sanzioni.md",
            "10-prova-pratica-casi-professionali.md",
        ],
    },
    {
        "code": "M-SA03",
        "book_id": "m-sa03-dirigenza-medica-sanitaria",
        "dir": WIKI / "moduli" / "m-sa03-dirigenza-medica-sanitaria" / "chapters",
        "files": [
            "01-profili-requisiti-prove-dirigenza-sanitaria.md",
            "02-programmazione-sanitaria-organizzazione-servizi.md",
            "03-linee-guida-appropriatezza-decisioni-cliniche.md",
            "04-governo-clinico-hta-qualita-accreditamento-rischio.md",
            "05-epidemiologia-sanita-pubblica-dirigenza.md",
            "06-dirigenza-medica-discipline-casi.md",
            "07-dirigenza-sanitaria-non-medica-discipline-casi.md",
        ],
    },
    {
        "code": "M-SA04",
        "book_id": "m-sa04-tecnici-sanitari-prevenzione",
        "dir": WIKI / "moduli" / "m-sa04-tecnici-sanitari-prevenzione" / "chapters",
        "files": [
            "01-profili-tslb-tsrm-requisiti-prove-responsabilita.md",
            "02-tslb-processo-laboratorio-qualita-biosicurezza.md",
            "03-tsrm-imaging-dosimetria-radioprotezione.md",
            "04-tecnologie-dispositivi-apparecchiature-rischio.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-07 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-07", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-07", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-07", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-07", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-07", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-07", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-07 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-07 · Fine</h2>
...
<head>
    ...
    <title>VOL-07 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol07-{n}" data-volume="VOL-07" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-07 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-07 · Indice volume compilato</p>
            <h1>Sanità amministrativa e professioni sanitarie</h1>
        </div>
    </header>
    <main>
        <p class="meta"><a href="../index.html" style="color:#D4AF37;text-decoration:none;">← Catalogo</a> · Cartella = numero volume + slug file wiki · 01–25 pronti</p>
        <ol class="chapters">
{chr(10).join(items)}
        </ol>
        <footer>01–05 M-SA01 · 06–14 M-SA02 · 15–21 M-SA03 · 22–25 M-SA04</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "spec": "wiki/books/volumi/vol-07-sanita-amministrativa-professioni-sanitarie/planning/00-scheda-pipeline.md",
        "order": "M-SA01, M-SA02, M-SA03, M-SA04 — 25 capitoli",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-07 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-07")
