...
const CHAPTER_09_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  await page
    .getByRole("button", { name: /9\.\s*Contratti pubblici essenziali/i })
    .click()
  ...
  if (count < 4) {
    throw new Error("Meno di 4 pagine nella preview del capitolo 9")
  }

  await pages.nth(0).screenshot({ path: "artifacts/chapter09-page1-preview.png" })
  ...
  const diag = await page.evaluate(() => {
    ...
    const chapter9Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-09"))
    const visibleChapter9Images = chapter9Images.map((image) => image.getAttribute("src") || "")
    const imageLayouts = chapter9Images.map((image, index) => {
  ...
  if (diag.chapter9Images !== 6) {
    throw new Error(`Attese 6 immagini chapter-09, trovate ${diag.chapter9Images}`)
  }
  ...
  if (diag.imageLayoutIssues.length > 0) {
    throw new Error(`Immagini chapter-09 fuori dalla pagina A4: ${JSON.stringify(diag.imageLayoutIssues)}`)
  }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_09_ASSETS))
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_09_ASSETS, file), "utf8")
