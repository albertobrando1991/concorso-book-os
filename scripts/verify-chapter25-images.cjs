...
const CHAPTER_25_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Aggiornare il metodo dopo il libro" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 25")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter25-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter25Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-25"))
      const visibleChapter25Images = chapter25Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter25Images.map((image, index) => {
    ...
    if (diag.chapter25Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-25, trovate ${diag.chapter25Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-25 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_25_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-25, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_25_ASSETS, file), "utf8")
