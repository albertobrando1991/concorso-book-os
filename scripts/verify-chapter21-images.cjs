...
const CHAPTER_21_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Come scegliere i moduli integrativi" })
      .first()
      .click()
    ...
    if (count < 10) {
      throw new Error("Meno di 10 pagine nella preview del capitolo 21")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter21-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter21Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-21"))
      const visibleChapter21Images = chapter21Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter21Images.map((image, index) => {
    ...
    if (diag.chapter21Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-21, trovate ${diag.chapter21Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-21 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_21_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-21, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_21_ASSETS, file), "utf8")
