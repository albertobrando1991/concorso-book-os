"""Genera i deck HTML VOL-04 dai capitoli wiki (M-FC04 Giustizia e UPP)."""
...
MODULES = [
    {
        ...
        "dir": WIKI / "moduli" / "m-fc04-giustizia" / "chapters",
        "files": [
            "01-sistema-giustizia-visto-dal-candidato.md",
            "02-ministero-dipartimenti-amministrazioni-giustizia.md",
            "03-uffici-giudiziari-ordinamento-lavoro-ufficio.md",
            "04-ufficio-per-il-processo-struttura-progetto-flussi.md",
            "05-lavoro-aupp-fascicolo-udienza-ricerca-scheda.md",
            "06-processo-civile-operativo-upp-cancelleria.md",
            "07-processo-penale-operativo-upp-cancelleria.md",
            "08-servizi-cancelleria-registri-comunicazioni-certificazioni.md",
            "09-spese-giustizia-patrocinio-recupero.md",
            "10-casellario-carichi-pendenti-archivi-certificati.md",
            "11-unep-notificazioni-esecuzioni-protesti.md",
            "12-giustizia-digitale-pct-ppt-fascicolo.md",
            "13-giustizia-minorile-comunita-mediazione-riparativa.md",
            "14-amministrazione-penitenziaria-trattamento-istituti-esecuzione-esterna.md",
            "15-appendici-strumenti-finali.md",
            "16-conclusione-vol-04.md",
            "17-fonti-riferimenti-essenziali.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-04 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-04", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-04", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-04", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-04", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-04", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-04", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-04 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-04 · Fine</h2>
...
<head>
    ...
    <title>VOL-04 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol04-{n}" data-volume="VOL-04" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-04 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-04 · Indice M-FC04</p>
            <h1>Giustizia e Ufficio per il processo</h1>
        </div>
    </header>
    <main>
        <p class="meta"><a href="../index.html" style="color:#D4AF37;text-decoration:none;">← Catalogo</a> · Cartella = numero volume + slug file wiki · 01–17 pronti</p>
        <ol class="chapters">
{chr(10).join(items)}
        </ol>
        <footer>01–14 corpi M-FC04 · 15 appendici · 16 conclusione · 17 fonti</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        ...
        "spec": "wiki/books/vol-04-giustizia-upp/planning/00-scheda-pipeline.md",
        "order": "modulo unico M-FC04, capitoli 01–17",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-04 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-04")
