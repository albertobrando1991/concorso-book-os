...
const CHAPTER_32_ASSETS = path.join(
  process.cwd(),
  "wiki",
  "books",
  "il-metodo-bando",
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .filter({ hasText: "Il tuo sistema BANDO" })
      .first()
      .click()
    ...
    if (count < 8) {
      throw new Error("Meno di 8 pagine nella preview del capitolo 32")
    }

    await pages.nth(0).screenshot({ path: "artifacts/chapter32-page1-preview.png" })
    ...
    const diag = await page.evaluate(() => {
      ...
      const chapter32Images = images.filter((image) => (image.getAttribute("src") || "").includes("chapter-32"))
      const visibleChapter32Images = chapter32Images.map((image) => image.getAttribute("src") || "")
      const imageLayouts = chapter32Images.map((image, index) => {
    ...
    if (diag.chapter32Images !== EXPECTED_IMAGE_COUNT) {
      throw new Error(`Attese ${EXPECTED_IMAGE_COUNT} immagini chapter-32, trovate ${diag.chapter32Images}`)
    }
    ...
    if (diag.imageLayoutIssues.length > 0) {
      throw new Error(`Immagini chapter-32 non conformi nella pagina: ${JSON.stringify(diag.imageLayoutIssues)}`)
    }
...
async function auditSvgTextGeometry(browser) {
  const files = (await fs.readdir(CHAPTER_32_ASSETS))
    .filter((file) => file.endsWith(".svg"))
    .sort()

  if (files.length !== EXPECTED_IMAGE_COUNT) {
    throw new Error(`Attesi ${EXPECTED_IMAGE_COUNT} SVG chapter-32, trovati ${files.length}`)
  }
  ...
  for (const file of files) {
    const svg = await fs.readFile(path.join(CHAPTER_32_ASSETS, file), "utf8")
