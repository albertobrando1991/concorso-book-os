"""Genera i deck HTML VOL-09 dai capitoli wiki (M-TR02 Appalti, PNRR e fondi UE)."""
...
SLIDES = ROOT / "slides" / "VOL-09"
VOLUME = "VOL-09"

MODULES = [
    {
        "code": "M-TR02",
        "book_id": "m-tr02-appalti-pnrr-fondi-ue",
        "dir": WIKI / "moduli" / "m-tr02-appalti-pnrr-fondi-ue" / "chapters",
        "files": [
            "01-quattro-profili-ciclo-integrato.md",
            "02-governance-rup-fasi-team-qualificazione.md",
            "03-strategia-fabbisogni-programmazione.md",
            "04-progettazione-gara-documenti.md",
            "05-sotto-soglia-procurement-operativo.md",
            "06-ecosistema-digitale-pad-pcp-bdncp-fvoe.md",
            "07-consip-mepa-aq-sdapa-asp.md",
            "08-governo-esecuzione.md",
            "09-rimedi-precontenzioso-contenzioso.md",
            "10-pnrr-milestone-target-regis.md",
            "11-gestione-finanziaria-rendicontazione-controlli.md",
            "12-dnsh-cam-procurement-sostenibile.md",
            "13-project-management-pubblico.md",
            "14-laboratorio-atti-casi-simulazioni.md",
        ],
    },
]
...
def build_deck(ch: dict, next_ch: dict | None, total: int) -> str:
    ...
    slides.append(
        f"""            <section class="slide ancora active">
                ...
                <div class="pad">
                    <p class="kicker reveal">VOL-09 · Capitolo {n}</p>
...
"""
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-09", f"02 / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-09", f"03 / {total:02d}"))
    ...
    for title_s, body_s in nuclei[:9]:
        ...
        slides.append(slide(kind, "\n".join(parts), "Capitale Personale · VOL-09", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("pearl", inner, "Capitale Personale · VOL-09", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ivory", inner, "Capitale Personale · VOL-09", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    slides.append(slide("ancora", inner, "Capitale Personale · VOL-09", f"{len(slides)+1:02d} / {total:02d}"))
    ...
    if next_ch:
        ...
        nxt_foot = f"VOL-09 · Capitolo {next_ch['n']}"
    else:
        nxt_inner = """                    <p class="kicker reveal">Dal volume al bando</p>
                    <hr class="gold-rule reveal d1" style="margin: 28px 0 36px;">
                    <h2 class="h-display reveal d2" style="font-size: 64px; max-width: 1500px;">VOL-09 · Fine</h2>
...
<head>
    ...
    <title>VOL-09 · Capitolo {n} — {esc(title)}</title>
...
<body data-edit-key="cp-vol09-{n}" data-volume="VOL-09" data-chapter="{n}" data-chapter-title="{esc(title)}" data-source="{esc(source)}">
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>VOL-09 — Capitoli in ordine · Slide staff</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
...
<body>
    <header>
        ...
        <div>
            <p class="kicker">VOL-09 · Indice volume compilato</p>
            <h1>Appalti, PNRR e procurement</h1>
        </div>
    </header>
    <main>
...
{chr(10).join(items)}
        ...
        <footer>01–05 ciclo gara · 06–07 digitale/Consip · 08–09 esecuzione/rimedi · 10–13 PNRR/finanza/PM · 14 laboratorio</footer>
    </main>
</body>
</html>
"""
...
def main() -> None:
    ...
    manifest = {
        "volume": "VOL-09",
        "volume_title": "Appalti, PNRR e procurement",
        "spec": "wiki/books/volumi/vol-09-appalti-pnrr-procurement/planning/00-scheda-pipeline.md",
        "order": "modulo unico M-TR02, capitoli 01–14",
        "chapters": chapters,
    }
    ...
    for i, ch in enumerate(chapters):
        ...
        print(f"VOL-09 {ch['n']}  {slides_n} slide  {ch['folder']}")
    print(f"generati {len(chapters)} deck VOL-09")
